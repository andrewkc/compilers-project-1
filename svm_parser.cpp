...
SVM* Parser::parse() {
  current = scanner->nextToken();
  if (check(Token::ERR)) {
      cout << "Scanner error - invalid character" << endl;
      exit(0);
  }
  ...
  while (current->type != Token::END) {
    ...
    sl.push_back(instr);
    // what do we do with the instruction?
  }  
  if (current->type != Token::END) {
    cout << "End-of-input expected, found " << current << endl;
  }
...
Instruction* Parser::parseInstruction() {
  ...
  if (match(Token::SKIP) || match(Token::POP) || match(Token::DUP) ||
    ...
    ttype = previous->type;
    if (!match(Token::NUM)) {
      cout << "NUM was expected after " << Token::token_names[ttype] << endl;
      exit(0);
    }
    ...
    ttype = previous->type;
    if (!match(Token::ID)) {
      cout << "ID expected after " << Token::token_names[ttype] << endl;
      exit(0);
    }   
    jmplabel = previous->lexema;
  } else {
    cout << "Error: Could not find match for " << current << endl;  
    exit(0);
  }
